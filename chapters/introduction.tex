...
for (int i = 0; i <= 5; ++i) {
	...
	\end{minted}
	could be analysed to determine that within the loop the variable \code{i} belongs to the interval $[0, 5]$, the variable \code{j} to $[0, 10]$ and so that the access to the array \code{a} may happen out of bounds.
