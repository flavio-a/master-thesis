...
The rationale behind this definition is that the analysis shouldn't get to $\bot$ because, as stated in the previous chapter, recovery from it is rare. This definition exactly prevents this when it's caused only by imprecisions of abstracted functions.
...
For the reminder of this section, we assume there is an under-approximation Galois insertion $\pow(C) (\alpha, \id_A) A$. Moreover, we say an element $S \in \pow(C)$ is \textit{representable} if it belongs to $A$, or equivalently if $\alpha(S) = S$.
